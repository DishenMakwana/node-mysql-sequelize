...
const createNewUser = catchAsync(async (req, res) => {
  ...
  if (registration) {
    ...
    await Promise.all([
      ...
      createAddress(req.body, user_id, USER_ADMIN),
    ]);

    if (req.body.userwise_pricing) {
      await addUserWisePricing(
        user_id,
        req.body.userwise_pricings,
        CUSTOM_PRICE
      );
    }

    await addRemainingUserWisePricing(user_id);
...
const editUser = catchAsync(async (req, res) => {
  ...
  await Promise.all([
    ...
    updateUserAddress(req.body, req.params.id, addressData.id, USER_ADMIN),
    deleteUserWise(req.params.id),
  ]);

  if (req.body.userwise_pricing) {
    await addUserWisePricing(req.params.id, req.body.userwise_pricings, 1);
  }

  await addRemainingUserWisePricing(req.params.id);
...
const getAllConfig = catchAsync(async (req, res) => {
  ...
  const config = await getAllConfigs(term);

  return sendMessage(
    { code: statusCodes.OK.code, data: config },
    messages.ALL_CONFIGS_LIST,
    statusCodes.OK.code,
    res,
    config
  );
...
const getConfig = catchAsync(async (req, res) => {
  const config = await getConfigs(req.query.type);
